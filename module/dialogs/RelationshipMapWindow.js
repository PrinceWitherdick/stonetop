...
export class RelationshipMapWindow extends StonetopDialog {
	 ...
	 *
	 * ⚠ `changed.flags[SYSTEM_ID]` in BRACKETS, never a dotted path. This package's id happens to
	 * be `stonetop_pwd`, where a dotted read would parse, but the same source ships under the
	 * hyphenated `stonetop-pwd`, and there a dotted `changed.flags.stonetop-pwd` parses as a
	 * subtraction and throws — inside a global hook, which takes down every other listener
	 * registered on it. See hooks/CondemnedTag.js.
	 */
	_wireSync() {
