// Sending a foe at a hero: the GM drags a foe's row in the Fight tab (or window) onto a hero's row, and
// the foe's token steps into the nearest free space touching that hero.
//
...
//
// THE NEAREST FREE SPACE. Every spot whose footprint touches the hero without covering them is a
// candidate; one that would cover another token, or hang off the scene, is not. Of the rest, the one
// closest to where the foe already stands wins, so a foe coming from the east arrives on the hero's
// east side. With no room at all, nothing moves and the GM is told.
//
...
/**
 * Move one fighter's token up against a fighter on the other side. GM only, and only on the scene
 * on the canvas, where the move can be seen.
 *
...
export async function sendAgainst(combat, moverId, targetId, { scene = globalThis.canvas?.scene, notify = globalThis.ui?.notifications } = {}) {
	...
	if (!game?.user?.isGM || !combat || !scene || canvas?.scene?.id !== scene.id || moverId === targetId) return false;
