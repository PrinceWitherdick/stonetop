 ...
 *
 * ⚠ Bracketed and built from SYSTEM_ID, never written out. This package's id happens to be
 * `stonetop_pwd`, where a dotted read would parse, but the same source ships under the
 * hyphenated `stonetop-pwd`, and there a dotted `flags.stonetop-pwd.portraitFrame` parses as a
 * subtraction and throws "pwd is not defined". Every display surface goes through here rather
 * than spelling the read out, so a scope rename moves one literal instead of six — and six that
 * silently resolve to `undefined` would revert every hand-chosen crop to the blind top slice
