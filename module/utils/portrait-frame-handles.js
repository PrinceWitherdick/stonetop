 ...
 *
 * ⚠ Every property read uses BRACKETS: `flags?.["stonetop_pwd"]?.x`. This package's id parses
 * dotted, but the same source ships under the hyphenated `stonetop-pwd`, where a dotted
 * `flags.stonetop-pwd.x` parses as a subtraction and fails at runtime with "pwd is not defined".
 * Nothing lints it. Dotted paths are fine inside string literals, which is why every one of
 * those is built from SYSTEM_ID rather than written out.
 *
