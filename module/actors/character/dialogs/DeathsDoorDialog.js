...
export class DeathsDoorDialog extends StonetopDialog {
	constructor(character, onDone, options = {}) {
		...
		this._step = this._fatePending ? "result" : "roll";   // "roll" | "result" | "fate"
	}
	 ...
	 */
	_mood() {
		if (this._step === "fate" || this._fatePending) return _MOODS.door;
	...
	/**
	 * Roll the move through the shared engine, so the card matches every other move card
	 * (tier outcomes, advantage pills, the GM's Shift Up/Down, XP on a miss — Death's Door
	 * doesn't say otherwise, so the standard "mark XP on a 6-" applies).
	 */
	async _onRoll() {
	...
	async _onTakeInsert(slug) {
		...
		this._step = "result";
		await this._post(name, `<p><strong>${escHtml(this._actorName)}</strong> refuses the Last Door and gains the <strong>${escHtml(name)}</strong> insert.</p>
			<p class="stonetop-dying-trigger">${slug === "thrall"
				? "Name your master, and the Mark it leaves on you."
				: "Choose your Terrible Purpose and your first Consequence."} They're on the Post-Death tab.</p>`);
		ui.notifications?.info?.(`${name} insert added. Fill it in on the Post-Death tab.`);
		this.render(true);
	}
