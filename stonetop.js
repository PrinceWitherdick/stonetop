...
function _cardMoveName(message) {
	return message.getFlag("stonetop_pwd", "move") ?? null;
}
...
async function _syncArcanumIdentification(message, actor, total) {
	const slug = message.getFlag("stonetop_pwd", "arcanum");
...
function _wireNeverAtALoss(message, html, actor) {
	...
	const chosen = message.getFlag("stonetop_pwd", "knowThingsXp") ?? null;
	for (const btn of buttons) {
		...
		btn.addEventListener("click", async () => {
			...
			try {
				await message.setFlag("stonetop_pwd", "knowThingsXp", choice);
...
function _wireLogbook(message, html, actor, card) {
	if (message.getFlag("stonetop_pwd", "knowThingsUpgrade")) return;
	...
	btn.addEventListener("click", async () => {
		...
		try {
			...
			await message.update({
				...
				flags:  { "stonetop_pwd": { knowThingsUpgrade: LOGBOOK } },
			});
			await ChatMessage.create({
