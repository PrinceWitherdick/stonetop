...
describe("CharacterLedger", () => {
	...
	it("never logs the Actor link a follower sweep writes back, whichever follower it is for", async () => {
		...
		const entries = await CharacterLedger.entriesForActorUpdate(actor, {
			"flags.stonetop_pwd.initiateDetails.enfys.actorUuid": "Actor.E0FGjEd91XwUD7Jc",
			"flags.stonetop_pwd.initiateDetails.afon.actorUuid": "Actor.si3Eu6QXhkPwDnIJ",
			"flags.stonetop_pwd.beastDetails.mule.actorUuid": "Actor.aaaaaaaaaaaaaaaa",
			"flags.stonetop_pwd.animalCompanion.details.actorUuid": "Actor.bbbbbbbbbbbbbbbb",
			"flags.stonetop_pwd.crew.details.actorUuid": "Actor.cccccccccccccccc",
		});
		expect(entries.map(e => e.action)).toEqual([]);
	});
