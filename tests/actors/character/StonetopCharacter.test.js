...
describe("StonetopCharacter.onRoll", () => {
	...
	it("reads the sticky mode off the flag and writes it only when told to", async () => {
		...
		expect(actor.setFlag).not.toHaveBeenCalledWith("stonetop_pwd", "rollMode", expect.anything());

		await char.setRollMode("adv");
		expect(actor.setFlag).toHaveBeenCalledWith("stonetop_pwd", "rollMode", "adv");
		// And junk on the way in never reaches the flag.
		await char.setRollMode("def");
		expect(actor.setFlag).toHaveBeenCalledWith("stonetop_pwd", "rollMode", "normal");
	});
});
